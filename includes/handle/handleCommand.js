         ...
         fs.readdir(folderPath, (err, files) => {
           ...
           if (latestFile) {
             ...
             if (checker.bestMatch.rating >= 0.5) {
              ...
              const formattedUptime = `${hours < 10 ? '0' + hours : hours}:${minutes < 10 ? '0' + minutes : minutes}:${seconds < 10 ? '0' + seconds : seconds}`;
              const currentTime = moment.tz("Asia/Ho_Chi_Minh").format("HH:mm:ss DD/MM/YYYY");
              
              const message = `[ Oops! Lệnh không tồn tại!✨️ ]\n-----------------------------------------\n➠ Lệnh gần giống có phải là: ${threadSetting.hasOwnProperty("PREFIX") ? threadSetting.PREFIX : PREFIX}${checker.bestMatch.target}\n\n🌀 Lệnh vừa tải là: ${newFile}\n⚡️ Dùng ${(threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX}menu để xem tất cả lệnh!\n------------------------------------------\n⏳️ Uptime của ${(!global.config.BOTNAME ? "bot" : global.config.BOTNAME)} là: ${formattedUptime}\n⏰️ Thời gian hiện tại: ${currentTime}`;
              
              api.sendMessage(message, event.threadID, async (err, info) => {
                await new Promise(resolve => setTimeout(resolve, 60 * 1000)); // 10 phút = 10 * 60 * 1000 ms
                 return api.unsendMessage(info.messageID);
               }, event.messageID);
             }
           }
