...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command['onLoad']) try {
                ...
                throw new Error('[ 𝗖𝗠𝗗 ] -  Không thể load module, lỗi: ' + JSON.stringify(error), 'error');
            }
    ...
    if (errorList.length != 0) api.sendMessage('[ 𝗖𝗠𝗗 ] -  Những lỗi vừa xảy ra khi hệ thống loading: ' + errorList.join(' '), threadID, messageID);
    api.sendMessage('[ 𝗖𝗠𝗗 ] -  ✅ Load thành công ' + (moduleList.length - errorList.length) +' lệnh!\n⏩️ Cụ thể lệnh:\n\n'+moduleList.join(', ') + '.js ', threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
    ...
    return api.sendMessage(`[ 𝗖𝗠𝗗 ] -  Thành công hủy ${moduleList.length} lệnh!`, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
  
    if (event.senderID != global.config.NDH[0]) return api.sendMessage(`[ 𝗖𝗠𝗗 ] -  Không đủ quyền hạn dùng lệnh!`, event.threadID, event.messageID)
    ...
    switch (args[0]) {
      ...
      break;
        ...
        case "load": {
            if (moduleList.length == 0) return api.sendMessage("[ 𝗖𝗠𝗗 ] -  Tên module không cho phép bỏ chống ⚠️", threadID, messageID);
        ...
        case "info": {
            ...
            if (!command) return api.sendMessage("[ 𝗖𝗠𝗗 ] -  Module này không tồn tại ⚠️", threadID, messageID);

            const { name, version, hasPermssion, credits, cooldowns, dependencies } = command.config;

            return api.sendMessage(
                "====== " + name.toUpperCase() + " ======\n" +
                "- Lệnh tạo bởi: " + credits + "\n" +
                "- Phiên bản: " + version + "\n" +
                "- Yêu cầu quền hạn: " + ((hasPermssion == 0) ? "Người dùng" : (hasPermssion == 1) ? "Quản trị viên" : "Support" ) + "\n" +
